...
import { getApplicationsByReceiver } from '../services/applicationService';
...
export default function ApplicationManagePage() {
    ...
    const [filter, setFilter] = useState<'all' | 'AVAILABLE' | 'ADOPTED' | 'HIDDEN' | 'RECEIVED'>('all');
    const [viewingApplication, setViewingApplication] = useState<AdoptionApplication | null>(null);
    const [isAdoptModalOpen, setIsAdoptModalOpen] = useState(false);
    const [adoptingApplicationId, setAdoptingApplicationId] = useState<string | null>(null);
    const [receiverEmail, setReceiverEmail] = useState('');
    const [adoptError, setAdoptError] = useState<string | null>(null);
    const [isAdopting, setIsAdopting] = useState(false);

    const { user, isAuthenticated } = useAuthStore();
    const { fetchMyApplications, getApplicationsByOwnerId, addApplication, updateApplicationStatus, deleteApplication, hideApplication, adoptApplication } = useApplicationStore();
    ...
    useEffect(() => {
        ...
        fetchMyApplications();
        // Fetch applications received by this user
    ...
    const fetchReceivedApplications = async () => {
        try {
            const received = await getApplicationsByReceiver();
            setReceivedApplications(received.map(app => ({
                id: app.applicationId,
                petId: app.pet.petId,
                ownerId: app.author.userId,
                owner: app.author,
    ...
    const openAdoptModal = (applicationId: string) => {
        setAdoptingApplicationId(applicationId);
        setReceiverEmail('');
        setAdoptError(null);
    ...
    const handleAdoptSubmit = async () => {
        if (!adoptingApplicationId || !receiverEmail.trim()) return;

        setIsAdopting(true);
        setAdoptError(null);

        try {
            // Gọi adoptApplication với receiverMail string
            const result = await adoptApplication(parseInt(adoptingApplicationId), receiverEmail.trim());

            if (result) {
                // Success - close modal and refresh data
                setIsAdoptModalOpen(false);
                setAdoptingApplicationId(null);
                setReceiverEmail('');
                setAdoptError(null);
            ...
            // Handle error - extract error message from different possible formats
            let errorMessage = 'An error occurred while processing adoption.';
    ...
    const handleStatusUpdate = async (applicationId: string, newStatus: 'AVAILABLE' | 'ADOPTED' | 'HIDDEN') => {
        ...
        if (newStatus === 'HIDDEN') {
            ...
            // Open modal for email input instead of direct API call
            openAdoptModal(applicationId);
            return; // Exit early, modal will handle the actual API call
    ...
    const filteredApplications = filter === 'RECEIVED'
    ...
    const availablePetsForApplication = userPets.filter(pet =>
        !applications.some(app => app.petId === pet.id && app.status === 'AVAILABLE')
    );
...
- Long-term commitment plans"
                ...
                {/* Adopt Modal - Email Input */}
                {isAdoptModalOpen && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                        <div className="bg-white rounded-2xl max-w-md w-full p-6">
                            <h2 className="text-2xl font-bold text-amber-900 mb-4">Mark as Adopted</h2>

                            <p className="text-amber-700 mb-6">
                                Please enter the email address of the person who is adopting this pet.
                            </p>
                            ...
                            {adoptError && (
                                <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <p className="text-sm text-red-800">{adoptError}</p>
                                    </div>
                                </div>
                            )}

                            <form onSubmit={(e) => { e.preventDefault(); handleAdoptSubmit(); }}>
                                <div className="mb-6">
                                    <label className="block text-sm font-medium text-amber-900 mb-2">
                                        Receiver Email *
                                    </label>
                                    ...
                                    />
                                    <p className="text-xs text-amber-600 mt-1">
                                        This email will be used to notify the adopter and record the adoption.
                                    </p>
                                </div>
                                ...
                                <div className="flex gap-4">
                                    <button
                                        type="submit"
                                        disabled={!receiverEmail.trim() || isAdopting}
                                    ...
                                    <button
                                        ...
                                        className="flex-1 bg-gray-200 text-gray-800 py-3 rounded-lg font-medium hover:bg-gray-300 transition disabled:opacity-50"
                                        disabled={isAdopting}
                                    >
                                        Cancel
                                    </button>
                ...
                {viewingApplication && (
                    ...
                    <span className={`px-4 py-2 rounded-full text-sm font-medium ${getStatusColor(viewingApplication.status)}`}>
                      ...
                      Posted on {formatDate(viewingApplication.createdAt)}
                                        ...
                                        <span className="text-sm text-amber-600">
                      Receive by {viewingApplication.receiver.email}
                    </span>
                                    </div>
                                </div>
